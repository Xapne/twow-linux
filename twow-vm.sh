...
if [ -d twow/.git ]; then
  ...
  tar czf twow-kitfiles-backup.tgz twow/twow.sh twow/lib twow/server/*.sh 2>/dev/null || true
  git -C twow init -q
...
# The kit names its own dependencies (the DEPS table in twow.sh is the
...
pkgs=$(twow/twow.sh deps --packages)
case $pkgs in
  ""|*/*|*"
...
convert() {
  ...
  while :; do
    ...
    # sshd runs this very command line, "twow.sh" and all, so a plain
    ...
    poll=$(vm 'tail -n 30 twow/setup.log 2>/dev/null | tr -d "\r"; pgrep -f "[t]wow.sh setup" >/dev/null && echo "==ALIVE==" || echo "==DEAD=="' 2>/dev/null) || poll=""
    [[ -z "$poll" ]] && continue
...
$(grep -A3 '\[error\]' <<<"$tail")"
    ...
    if [[ "$alive" == no ]]; then
      ...
      vm 'grep -q "conversion complete" twow/setup.log' 2>/dev/null \
        || { printf '\n'; die "twow.sh stopped early - last log lines:
...
open_forwards() {
  ...
  vm "cd twow && ./twow.sh realm $addr --bind 0.0.0.0" >/dev/null 2>&1 \
...
# accounts too.
first_run() {
  vmtty 'cd twow && ./twow.sh firstrun' \
...
handoff() {
  ...
  sleep 3
  exec ssh -t "${SSHOPTS[@]}" turtle@127.0.0.1 'cd twow && ./twow.sh run'
}
...
status() {
  ...
  { vm 'cd twow && ./twow.sh status' 2>/dev/null || true; } | relay_kit
  ui_outro "work dir: $WD"
}
...
stop() {
  ...
  { vm 'cd twow && ./twow.sh stop' 2>&1 || true; } | relay_kit
...
# Written by twow-vm.sh. Its presence marks this directory as holding a VM
# that twow-vm.sh created, and is what permits 'twow-vm.sh destroy' to
# remove it.
name=$VM_NAME
disk=$DISK
EOF
...
main() {
  case "${1:-}" in
    ...
    console) exec ssh -t "${SSHOPTS[@]}" turtle@127.0.0.1 "cd twow && ./twow.sh run ${2:-}";;
    tune)    exec ssh -t "${SSHOPTS[@]}" turtle@127.0.0.1 'cd twow && ./twow.sh interactive';;
    "") ;;
